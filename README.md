...
- **설정** — 각 옵션마다 상세 설명. 읽기 방향 / 페이지 표시(자동·한 장·양면) / 양면 스캔본 분할 토글 /
...
- 드래그 위치를 따라오는 3D 종이책 페이지 넘김 (`eschao/android-PageFlip`, `pageflip/` 로 벤더링).
  가로 중심선을 넘겨도 **드래그 중엔 안 넘어가고, 손을 뗀 위치 기준**으로 완료/되돌림 판정.
- **창(window) 방식 로딩**: 파일 목록만 먼저 읽고, 현재 페이지 앞뒤 몇 장만 백그라운드로 받아 디코드.
  아직 안 받은 페이지는 **회색**으로 보이다 도착 시 교체. 미리 불러올 장 수는 설정에서 조절(1~12).
...
- **폴드/회전으로 해상도가 바뀌면 화면을 다시 구성**해 즉시 대응(현재 페이지 유지, 양면 페어 왼쪽으로 스냅).
