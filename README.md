...
- **접힌 종이의 뒷면 텍스처는 좌우 반전해서 올린다**(`PageImageProvider.backBitmap`): eschao 는 뒷면을
  '종이 좌표' 그대로 찍어서, 넘김이 끝나 평평해진 순간의 그림이 정지 화면과 좌우가 반대였다. 여백이
  좌우 대칭이던 시절엔 티가 안 났지만, 여백을 바깥쪽으로 몰고 나서는 **넘김 중에는 검은 여백이
  화면 가운데**에 있다가 끝나는 순간 바깥으로 튀어 음영이 번쩍이는 것처럼 보였다.
