...
- **가장자리 탭 넘김은 합성 드래그**로 처리한다. 실제 책처럼 아래 모서리를 잡고 대각선 위로 휙 채듯이
  ...
  놓기는 기존 스크롤러 그대로.
- **넘김이 끝날 때 접힘 그림자는 서서히 사라진다**(`PageFlip.fadeShadowNearEnd`): 접힘 밑동이 반대쪽
  가장자리(양면 모드는 스파인)에 닿는 지점(페이지 폭의 90%)부터 smoothstep 으로 그림자 알파를 줄여,
  페이지가 다 넘어갈 무렵엔 이미 없다. 예전엔 짙은 그림자가 남은 채 애니메이션이 끝나고 다음 프레임이
  그림자 없는 정지 화면이라 펼친 화면 가운데 음영이 번쩍 사라지는 것처럼 보였다.
  그림자 자체는 설정에서 끌 수 있다(`Prefs.flipShadow` → `PageFlip.enableShadow`).
- **손 뗀 뒤 넘김 시간은 남은 거리에 비례**한다(`MIN_FLIP_DURATION` 220ms ~ 900ms). 거의 다 넘긴 뒤
  놓아도 항상 900ms 를 쓰느라 마지막에 느릿하게 멈칫하던 것을 없앴다.
