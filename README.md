...
- **넘김이 끝날 때 접힘 그림자는 서서히 사라진다**(`PageFlip.fadeShadowNearEnd`): 접힘 밑동이 반대쪽
  가장자리(양면 모드는 스파인)에서 페이지 폭의 30% 안으로 들어오면 그림자 알파를 거리에 비례해 줄인다.
  예전엔 짙은 그림자가 남은 채 애니메이션이 끝나고 다음 프레임이 그림자 없는 정지 화면이라,
  펼친 화면 가운데 음영이 번쩍 사라지는 것처럼 보였다(특히 가장자리 탭 넘김처럼 빠를 때).
